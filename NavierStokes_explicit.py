...
# Set convection form
a_conv = convection_form(convection, **vars())
assert(isinstance(a_conv, dict))
...
while t < (T - tstep*DOLFIN_EPS) and not stop:
    ...
    while err > max_error and inner_iter < num_iter:
        ...
        if inner_iter == 1:
            ...
            for ui in u_components:
                b[ui] = assemble(-a_conv[ui]*dx, tensor=b[ui])
                b[ui].axpy(1., b0[ui])           # add body force
                b[ui].axpy(1., A_rhs*x_1[ui])    # Add transient and diffusion

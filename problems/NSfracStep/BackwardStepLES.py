...
t1 = Polygon([Point(0.08, 0.005), Point(0.1, 0.005), Point(0.1,0.006)])
t2 = Polygon([Point(0.08, 0.01), Point(0.1, 0.009), Point(0.1,0.01)])

domain = r2-r3-r1-t1-t2

mesh = generate_mesh(domain, 1300)
...
def pre_solve_hook(mesh, nut_, velocity_degree, **NS_namespace):
    ...
    CSGSFile = File("results/bwfstep/Cs.pvd")
    v_file = File("results/bwfstep/U.pvd")
    JLM_file = File("results/bwfstep/JLM.pvd")
    JMM_file = File("results/bwfstep/JMM.pvd")
    set_log_active(False)
    return dict(uv=Function(Vv), nutfile=nutfile, 
            CSGSFile=CSGSFile, v_file=v_file, JLM_file=JLM_file,
            JMM_file=JMM_file)
...
def temporal_hook(tstep, save_step, nut_, u_, nutfile, v_file, uv, 
        CSGSFile, Cs, JLM_file, JMM_file, JLM, JMM,  **NS_namespace):

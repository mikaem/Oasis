...
mesh = RectangleMesh(0,0,0.2,0.5, 250, 500)
...
def pre_solve_hook(mesh, nut_, velocity_degree, **NS_namespace):
    ...
    JLM_file = File("results/temp_box/JLM.pvd")
    JMM_file = File("results/temp_box/JMM.pvd")
    set_log_active(False)
    return dict(uv=Function(Vv), nutfile=nutfile, 
            CSGSFile=CSGSFile, v_file=v_file, T_file=T_file, JLM_file=JLM_file,
            JMM_file=JMM_file)
...
def temporal_hook(tstep, save_step, nut_, u_, nutfile, v_file, uv, 
        CSGSFile, Cs, T_file, JLM_file, JMM_file, JLM, JMM, Temp, **NS_namespace):

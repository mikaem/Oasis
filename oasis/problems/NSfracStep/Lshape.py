...
def problem_parameters(
    NS_parameters, commandline_kwargs, NS_expressions, **NS_namespace
):
    Re = 500.0
    nu = 1.0 / Re
    ...
    NS_expressions.update(dict(p_in=Expression("sin(pi*t)", t=0.0, degree=degree)))
...
def inlet(x, on_boundary):
    return near(x[1] - 1.0, 0.0) and on_boundary


def outlet(x, on_boundary):
    return near(x[0] - 1.0, 0.0) and on_boundary
...
def walls(x, on_boundary):
    return (
        near(x[0], 0.0)
        or near(x[1], 0.0)
        or (x[0] > 0.25 - 5 * DOLFIN_EPS and x[1] > 0.25 - 5 * DOLFIN_EPS)
        and on_boundary
    )
